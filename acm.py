...
class NominalACM:
    ...
    short_rate_proxy: pandas.Series
        Monthly aligned one-period short rate used everywhere the model needs
        the short rate. If not supplied by the user, the first maturity of
        `curve_monthly` is used.
    ...
    def __init__(
        ...
        short_rate_proxy: pandas.Series or single-column pandas.DataFrame, optional
            Optional proxy for the short rate of the model. If None is passed,
            the first maturity of `curve_monthly` is used. If passed, the
            proxy is interpreted as a one-period short rate in monthly log
            terms and is aligned to the monthly estimation sample. Inputs must
            use the same minimal format as the rest of the library:
                - pandas.Series with a DatetimeIndex
                - single-column pandas.DataFrame with a DatetimeIndex
            If the proxy has frequency higher than monthly, it is resampled to
            month-end by mean before alignment.
        """
        ...
        self.short_rate_proxy = self._prepare_short_rate_proxy(short_rate_proxy)
    ...
    def _assertions(curve, curve_m, selected_maturities):
        ...
        if curve_m is not None:
            ...
            # Monthly frequency
            assert pd.infer_freq(curve_m.index) in {'M', 'ME'}, \
                "`curve_m` must have a DatetimeIndex with monthly frequency"

    def _prepare_short_rate_proxy(self, short_rate_proxy):
        if short_rate_proxy is None:
            proxy = (self.curve_monthly.iloc[:, 0] / 12).copy()
            proxy.name = self.curve_monthly.columns[0]
            return proxy

        if isinstance(short_rate_proxy, pd.Series):
            proxy = short_rate_proxy.copy()
        elif isinstance(short_rate_proxy, pd.DataFrame):
            if short_rate_proxy.shape[1] != 1:
                raise ValueError(
                    "`short_rate_proxy` DataFrame must have exactly one column"
                )
            proxy = short_rate_proxy.iloc[:, 0].copy()
        else:
            raise TypeError(
                "`short_rate_proxy` must be a pandas Series or DataFrame"
            )

        if not isinstance(proxy.index, pd.DatetimeIndex):
            raise TypeError(
                "`short_rate_proxy` must have a DatetimeIndex"
            )
        ...
        if proxy.isna().any():
            missing_dates = proxy.index[proxy.isna()].strftime('%Y-%m-%d').tolist()
            missing_dates = missing_dates[:5]
            raise ValueError(
                "`short_rate_proxy` does not cover the monthly estimation "
                f"sample. Missing dates include: {missing_dates}"
            )

        return proxy.rename(proxy.name or "short_rate_proxy")

    def _get_excess_returns(self):
        ttm = np.arange(1, self.n + 1) / 12
        log_prices = - self.curve_monthly * ttm
        rf = self.short_rate_proxy.shift(1)
